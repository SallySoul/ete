...
if ! (test -f $HOME/.cargo/bin/wasm-bindgen && test "`wasm-bindgen --version`" = "wasm-bindgen 0.2.48" ); then
  echo "wasm-bindgen not installed or out of date: installing"
  cargo install -f wasm-bindgen-cli --version 0.2.48
else
...
if [ -z "$RELEASE"  ]; then
  ...
  wasm-bindgen ../target/wasm32-unknown-unknown/debug/ete.wasm --out-dir . --no-typescript --no-modules
...
# RELEASE=1 ./build.sh
  ...
  wasm-bindgen ./target/wasm32-unknown-unknown/release/ete.wasm --out-dir . --no-typescript --no-modules &&
  wasm-opt -O3 -o optimized.wasm ete_bg.wasm  &&
  mv optimized.wasm ete_bg.wasm
fi
